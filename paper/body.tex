...
An experiment creates a controller for a model and a grid of candidate designs (\texttt{createController}), authors ordinary jsPsych trials that read the current design from the controller (\texttt{evaluateDesignVariable}) and report the outcome from the trial's \texttt{on\_finish} (\texttt{recordResponse}), and then wraps those trials into the adaptive timeline (\texttt{createTimeline}).

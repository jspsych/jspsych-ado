...
These artifacts are compiled from `../exponential.stan` and committed so
...
- **Artifact names:** kept as `compiled/main.js` + `compiled/main.wasm` — the file
  names stay `main.js` and `main.wasm` because `main.js` hardcodes loading its
  sibling `main.wasm`
...
curl -s "https://stan-wasm.flatironinstitute.org/download/$ID/main.js"   -o compiled/main.js
curl -s "https://stan-wasm.flatironinstitute.org/download/$ID/main.wasm" -o compiled/main.wasm

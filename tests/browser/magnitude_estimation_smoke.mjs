...
async function runMode(browser, baseUrl, spec) {
  ...
  const result = await page.waitForFunction((task, nTrials) => {
  ...
  }, { timeout: spec.timeout, polling: 500 }, TASK, N_TRIALS).then((h) => h.jsonValue());
...
try {
  ...
  for (const spec of specs) {
    ...
    if (!r.errored) {
      note(r.choiceRows === N_TRIALS, `${mode}: ${N_TRIALS} choice trials recorded (got ${r.choiceRows})`);
      note(r.updateRows === N_TRIALS, `${mode}: ${N_TRIALS} update rows recorded (got ${r.updateRows})`);
      ...
      note(r.updateRowsWithMetrics === N_TRIALS, `${mode}: update rows carry ado_next_design_metrics`);
      // Continuous response: choice is a real number (log estimate), not a class index,
      // and there is no categorical label.
      note(typeof r.choice === "number" && Number.isFinite(r.choice), `${mode}: choice is a finite real number (got ${r.choice})`);
      note(typeof r.choiceRaw === "number" && Number.isFinite(r.choiceRaw), `${mode}: choice_raw (raw slider estimate) recorded (got ${r.choiceRaw})`);
      note(r.choiceLabel === null, `${mode}: choice_label is null for a continuous response (got ${r.choiceLabel})`);
      note(r.controllerMode === (mode === "random" ? "stan" : mode),
        `${mode}: controller_mode recorded (got ${r.controllerMode})`);
      note(typeof r.simB === "number" && typeof r.simEstimate === "number",
      ...
      if (mode === "stan" || mode === "random") {
        ...
        note(typeof r.postMeanLoga === "number" && typeof r.postMeanB === "number" &&
          typeof r.postSdB === "number" && typeof r.postMeanSigma === "number",

...
test("copies trial columns and injects N", () => {
  ...
  assert.deepEqual(build([{ a: 1, b: 2 }, { a: 3, b: 4 }]), { N: 2, a: [1, 3], b: [2, 4] });
});

test('"response" maps to the jsPsych choice; binary => no +1', () => {
  const build = makeStanDataBuilder({ stanData: { y: "response" }, responseSpace: { type: "binary" } });
...
test("{ from, index1: true } reads a renamed column and adds +1", () => {
  ...
  assert.deepEqual(build([{ target_index: 0, delta: 8 }, { target_index: 2, delta: 16 }]), {
    N: 2,
    target_index: [1, 3],
    delta: [8, 16],
  });
...
test("{ from } without index1 just renames a column (no offset)", () => {
  const build = makeStanDataBuilder({ stanData: { stan_x: { from: "x" } }, responseSpace: { type: "binary" } });
  assert.deepEqual(build([{ x: 5 }]), { N: 1, stan_x: [5] });
});

test("N must never be declared in the map", () => {
  assert.throws(
    () => makeStanDataBuilder({ stanData: { N: "n", y: "response" }, responseSpace: { type: "binary" } }),
    /must not declare `N`/
  );
});
